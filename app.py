            ...
            if st.session_state.get("show_generate_plan_button"):
                ...
                if col1.button(
                    ...
                    with st.spinner(
                        ...
                        user_story_to_save = st.session_state.get(
                            "user_story_input", ""
                        )
    ...
    if st.session_state.get("analysis_finished"):
        ...
        if (
            ...
            with st.expander(
                ...
                st.selectbox(
                    "Prioridade Padrão:", ["Medium", "High", "Low"], key="jira_priority"
                )
                st.text_input(
                    "Labels (separadas por vírgula):", "QA-Oraculo", key="jira_labels"
                )
                st.text_area(
                    "Descrição Padrão:",
            ...
            col_azure.download_button(
                ...
                excel_azure_bytes,
                file_name=gerar_nome_arquivo_seguro(
                    st.session_state.get("user_story_input", ""), "azure.xlsx"
                ),
...
def render_history_page():  # noqa: C901, PLR0912, PLR0915
    ...
    if st.session_state.get("confirm_delete_id"):
        ...
        st.warning(
            f"⚠️ Tem certeza que deseja excluir a análise {delete_id}? Esta ação não pode ser desfeita."
        )

...
def test_from_env_google_with_specific_key():
    ...
    with patch.dict(
        os.environ,
        {"LLM_PROVIDER": "google", "GOOGLE_API_KEY": "google-specific-key"},
        clear=True,
    ):
        settings = LLMSettings.from_env()
...
def test_from_env_openai_fallback_to_specific_key():
    ...
    with patch.dict(
        os.environ,
        {"LLM_PROVIDER": "openai", "OPENAI_API_KEY": "openai-specific"},
        clear=True,
    ):
        settings = LLMSettings.from_env()
...
def test_from_env_llama_fallback_to_specific_key():
    ...
    with patch.dict(
        os.environ,
        {"LLM_PROVIDER": "llama", "LLAMA_API_KEY": "llama-specific"},
        clear=True,
    ):
        settings = LLMSettings.from_env()
...
def test_from_env_gpt_provider_alias():
    ...
    with patch.dict(
        os.environ, {"LLM_PROVIDER": "gpt", "OPENAI_API_KEY": "gpt-key"}, clear=True
    ):
        settings = LLMSettings.from_env()
